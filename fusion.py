...
public class {{ FunctionId }}IRAPFormService implements FormService
{
                            ...
                            FunctionalContext functionalContext)
    {
        ...
        {
            Data data = new Data();
            String xidlog = utilsService.xlogCreate("{{ FunctionId.upper() }}C");
...
# === ÉTAPE 1 : ÉCRIRE LES .j2 DANS output_templates/ ===
def write_templates():
    for filename, content in TEMPLATES.items():
        path = os.path.join(OUTPUT_TEMPLATE_DIR, filename)
        with open(path, "w", encoding="utf-8") as f:
            f.write(content)
        print(f"📌 Template créé : {path}")

# === ÉTAPE 2 : GÉNÉRER LES .java DANS output/ ===
def generate_code(fct_name):
    FunctionId = capitalize_first(fct_name)
    functionId = fct_name.lower()

    env = Environment(loader=FileSystemLoader(OUTPUT_TEMPLATE_DIR), trim_blocks=True, lstrip_blocks=True)
...
if __name__ == "__main__":
    # Étape 1 : écrire les templates bruts
    write_templates()

    # Étape 2 : demander la fonction et générer le code
